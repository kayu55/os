...
echo -e " - Untuk Pasword Nya Bisa Tonton Di Youtube Aris Stya Channel "
echo -e " "
...
echo -e " "
echo -e "\033[0;32m Segera Hub. Admin 081450330727 "
echo -e " "
exit
fi
...
function install_xray() {
    ...
    wget -q -O /etc/haproxy/haproxy.cfg "${REPOS}lc/haproxy.cfg"
    wget -q -O /etc/nginx/conf.d/xray.conf "${REPOS}lc/xray.conf"
    curl -s "${REPOS}lc/nginx.conf" > /etc/nginx/nginx.conf 
...
wsoket_setup() {
    ...
    wget -q -O "$tun_conf" "${REPOSE}murah/rclone.conf"
